...
def get_parser():
    ...
    parser.add_argument('--arch', type=str, default='CHPDNet')
...
def train(train_loader, val_loader, model, optimizer, epoch):
    ...
    for i, (input, input_name, target, target_b, s_input, s_mask, subcls, class_name, img_cv2) in enumerate(train_loader):
        ...
        output, main_loss, aux_loss1 = model(s_x=s_input, que_name=input_name, s_y=s_mask, x=input, x_cv2=img_cv2, y_m=target, y_b=target_b, cat_idx=subcls, class_name=class_name)

        loss = main_loss + args.aux_weight1 * aux_loss1
...
def validate(val_loader, model):
    ...
    for e in range(db_epoch):
        for i, (input, input_name, target, target_b, s_input, s_mask, subcls, class_name, ori_label, ori_label_b,
            ...
            output, meta_out = model(s_x=s_input, x_cv2=img_cv2, que_name=input_name, s_y=s_mask, x=input,
