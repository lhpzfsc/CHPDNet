    ...
    for i in range(0, len(tmp_que_name)):
        ...
        for idx, label in enumerate(label_list):
            ...
            if label_id == que_class[i]:
                ...
                mean_value = torch.mean(trans_mat)
                # print(mean_value)
                H_trans_mat = trans_mat @ trans_mat.t()
                abs_D = torch.abs(H_trans_mat)
                thresholded = torch.relu(abs_D - mean_value)

                sign_D = torch.sign(H_trans_mat)
                trans_mat = sign_D * thresholded
